...
function toplegal_add_meta_box () {
	add_meta_box( 'guida-item-type', 'Directory', 'guida_items_type_box_field', 'guida_item', 'side', 'high' );
	/*--------------------------------------------------------------------*/
...
function guida_items_type_box_field ($post) {
	wp_nonce_field( basename(__FILE__), 'guida_items_type_mam_nonce' );
	$directory = maybe_unserialize( get_post_meta( $post->ID, 'guida-items-directory', true ) );
?>
	<div class="ui-widget">
   		<select id="guida-items-type" name="guida-items-type" style="width: 100%;">
...
function guida_items_type_save_data ($post_id) {
	$is_autosave = wp_is_post_autosave( $post_id );
    $is_revision = wp_is_post_revision( $post_id );
    $is_valid_nonce = ( isset( $_POST[ 'guida_items_type_mam_nonce' ] ) && wp_verify_nonce( $_POST[ 'guida_items_type_mam_nonce' ], basename( __FILE__ ) ) ) ? 'true' : 'false';

    if ( $is_autosave || $is_revision || !$is_valid_nonce ) {
        return;
    }

    if ( ! empty( $_POST['guida-items-type'] ) ) {
        update_post_meta( $post_id, 'guida-items-directory', $_POST['guida-items-type'] );
    } else {
        delete_post_meta( $post_id, 'guida-items-directory' );
    }
}
add_action('save_post', 'guida_items_type_save_data');
...
function guida_items_directory_box_field($post) {
	...
	$directory = maybe_unserialize( get_post_meta( $post->ID, 'guida-items-search-type', true ) );
	$guida_item_post_type = maybe_unserialize( get_post_meta( $post->ID, 'guida-items-directory', true ) );
...
?>
	...
	<div class="ui-widget" style="margin-top:2em; font-family:Arial">
		Studio:
		<div>
			<input type="text" id="guida-item-external-id-post" name="guida-item-external-id-post" value="<?php echo $extrn_id; ?>" style="width: 92%; float: left; padding: 2px 5px; margin: 0px;"/>
	...
	<?php if ( $guida_item_post_type == 'professionista' ): ?>
	<div class="ui-widget" style="margin-top:2em; font-family:Arial">
		Professionista:
		<div>
			<input type="text" id="guida-item-external-id-post" name="guida-item-external-id-post" value="<?php echo $extrn_id; ?>" style="width: 92%; float: left; padding: 2px 5px; margin: 0px;"/>
	...
	<div class="ui-widget" style="margin-top:2em; font-family:Arial">
	  	Result:
	  	<div id="log" style="width: 100%; overflow: auto;" class="ui-widget-content">
	  		<div id="items-st-prof" style="height: 200px; width: 100%; overflow: auto; border: 0px solid #666;">
		  		<?php foreach ($output as $value): ?>
		  			<div class="search-result-list"><a href="" id="<?php echo $value['id']; ?>"><?php echo $value['name']; ?></a></div>
		  		<?php endforeach; ?>
	  		</div>
	  		<div class="rcbMoreResults" id="ctl00_contenutoControlli_boxDettaglioCentroStudi1_gvProf_ctl00_ctl07_listStudiProf_MoreResultsBox" style="cursor: default;">
	  			<a id="ctl00_contenutoControlli_boxDettaglioCentroStudi1_gvProf_ctl00_ctl07_listStudiProf_MoreResultsBoxImage">select</a>
	  			<span></span>
	  		</div>
	  	</div>
	</div>
...
function guida_items_directory_save_data ($post_id) {
    ...
    if ( ! empty( $_POST['guida-items-directory'] ) ) {
        update_post_meta( $post_id, 'guida-items-search-type', $_POST['guida-items-directory'] );
    } else {
        delete_post_meta( $post_id, 'guida-items-search-type' );
    }
    ...
    if ( ! empty( $_POST['guida-item-external-id-post'] ) ) {
        ...
        $type = $_POST['guida-items-directory'];
        $search_word = $_POST['guida-item-external-id-post'];
